...
export function useKeyboard(optionsOrKey:string | KeyboardOptions,callback:(e:KeyboardEvent)=>void){
    ...
    let options:KeyboardOptions = Object.assign(defaultOptions,typeof optionsOrKey === "string" ? {key:optionsOrKey} : optionsOrKey);
    const {type = 'keydown',once} = options;
    useEventListener(document,type,(e:KeyboardEvent)=>{
