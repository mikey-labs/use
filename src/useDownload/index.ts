...
const saveAsAnchor = (target: string | Blob, fileName: string, anchor: HTMLAnchorElement): Promise<boolean> => {
 ...
 * save on ie 10+
 */
const saveInIE = (target: Blob, fileName: string): Promise<boolean> => {
    return new Promise((resolve) => {
        try {
...
export function useDownload(target: string | Blob, fileName: string): Promise<boolean> {
